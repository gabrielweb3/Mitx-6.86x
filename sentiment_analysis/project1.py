...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    if label*(theta.T@feature_vector + theta_0)>=1:
        Loss = 0
    else:
        Loss = np.abs(label - (theta.T@feature_vector + theta_0))
...
def perceptron_single_step_update(
    ...
    # Your code here
    # raise NotImplementedError
    if np.sign(label)!=np.sign(current_theta.T@feature_vector + current_theta_0):
...
def perceptron(feature_matrix, labels, T):
    ...
    #inicializo theta y theta_0 en cero
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for i in range(T):
        for j in get_order(feature_matrix.shape[0]):
            ...
            v_theta_0.append(theta_0)
                
    theta = np.mean(v_theta, axis=0)
    theta_0 = np.mean(v_theta_0)
    return (theta, theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    Returns:
        ...
        should be considered a positive prediction.
    """
    # Your code here
    #raise NotImplementedError
    nsamples = feature_matrix.shape[0]
    prediction = np.ones(nsamples)
    for i in range(nsamples):
        if (theta.T@feature_matrix[i,:] + theta_0)<=0:
            prediction[i] = -1
    return prediction
...
def classifier_accuracy(
    ...
    train_accuarcy = accuracy(train_prediction, train_labels)
    val_accuarcy = accuracy(val_prediction, val_labels)
    
    return (train_accuarcy, val_accuarcy)
...
def bag_of_words(texts, stop_words=None, remove_stopword=False):
    """
        ...
        for word in word_list:
            ...
            if binarize:
                if word in indices_by_word:
                    ...
                    feature_matrix[i, indices_by_word[word]] = 0
