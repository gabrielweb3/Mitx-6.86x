...
dictionary2 = p1.bag_of_words(train_texts, stop_words=stop_words, remove_stopword=True)
...
pred = np.sign(test_bow_features@theta + theta_0)
best_acc = p1.accuracy(pred, test_labels)
# print(best_acc)
#-------------------------------------------------------------------------------
...
### Wtih stopwords removed or binary features change
# Your code here
T=25
L=0.01
theta, theta_0 = p1.pegasos(train_bow_features, train_labels, T, L)
...
wordlist   = [word for (idx, word) in sorted(zip(dictionary2.values(), dictionary2.keys()))]
sorted_word_features = utils.most_explanatory_word(best_theta, wordlist)
print("Most Explanatory Word Features")
print(sorted_word_features[:10])
